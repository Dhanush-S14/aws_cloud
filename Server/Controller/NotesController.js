...
// Feature flag: disable image uploads when not using Cloudinary
const CLOUDINARY_ENABLED =
  Boolean(process.env.CLOUDINARY_CLOUD_NAME &&
    process.env.CLOUDINARY_API_KEY &&
    process.env.CLOUDINARY_API_SECRET) &&
  String(process.env.ENABLE_IMAGE_UPLOAD || "false").toLowerCase() ===
    "true";

// Configure Cloudinary only when explicitly enabled and all creds exist
...
export const uploadNoteImage = async (req, res) => {
  ...
  try {
    if (!CLOUDINARY_ENABLED) {
      return res
        .status(501)
        .json({ error: "Image upload is disabled on this server" });
    }
